...
stdenv.mkDerivation (finalAttrs: {
  ...
  postPatch = ''
    ...
    substituteInPlace Makefile.gnu \
      --replace-fail "pkg-config" "$PKG_CONFIG"
    substituteInPlace Source/FreeImage/PluginHDR.cpp --replace-fail \
      "snprintf(buffer, HDR_MAXLINE, \"%s\n\", info->comment);" \
      "snprintf(buffer, HDR_MAXLINE, \"%.*s\n\", HDR_MAXLINE - 2, info->comment);"
    substituteInPlace Source/FreeImageToolkit/JPEGTransform.cpp --replace-fail \
  ...
  buildInputs = [
    ...
    openjpeg
    libjpeg_turbo-freeimage
    libjpeg_turbo-freeimage.dev_private
    jxrlib
  ];
